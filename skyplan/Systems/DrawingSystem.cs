...
namespace skyplan.Systems {
	...
	public partial class DrawingSystem : UISystemBase {
		...
		private void HandleDrawStart(float sx, float sy) {
			...
			if (!m_Camera.ScreenToWorldXZ(sx, sy, out Vector2 xz)) return;
		...
		private Vector2 ShapeScreenCentroid(Shape s) {
			...
			foreach (var p in s.pts)
				sum += m_Camera.WorldToSVG(new Vector3(p.x, 0, p.y));
		...
		private string ShapeToJSON(Shape s) {
			...
			switch (s.type) {
				case "line": {
					...
					Vector2 p0 = m_Camera.WorldToSVG(new Vector3(s.pts[0].x, 0, s.pts[0].y));
					Vector2 p1 = m_Camera.WorldToSVG(new Vector3(s.pts[1].x, 0, s.pts[1].y));
					sb.Append(",\"tag\":\"line\"");
				...
				case "rect": {
					if (s.pts.Count < 2) return null;
					Vector2 a  = m_Camera.WorldToSVG(new Vector3(s.pts[0].x, 0, s.pts[0].y));
					Vector2 b  = m_Camera.WorldToSVG(new Vector3(s.pts[1].x, 0, s.pts[0].y));
					Vector2 c2 = m_Camera.WorldToSVG(new Vector3(s.pts[1].x, 0, s.pts[1].y));
					Vector2 d  = m_Camera.WorldToSVG(new Vector3(s.pts[0].x, 0, s.pts[1].y));
				...
				case "circle": {
					...
					for (int i = 0; i < N; i++) {
						...
						Vector2 sp = m_Camera.WorldToSVG(new Vector3(
				...
				case "free": {
					...
					for (int i = 0; i < s.pts.Count; i++) {
						Vector2 p = m_Camera.WorldToSVG(new Vector3(s.pts[i].x, 0, s.pts[i].y));
