...
const DrawingCanvasTransform: React.FC<DrawingCanvasTransformProps> = ({
	activeTool, shapesBaseline, preview, highlightId, svgSize, transform,
}) => {
	...
	return (
		...
		>
			{/* baseline shapes shifted to current camera via affine CSS transform */}
			<g transform={transform || undefined}>
				{shapesBaseline.map(s => renderShape(s, hasHighlight ? (s.id === highlightId ? '1' : '0.3') : undefined))}
			</g>
			{/* preview in current-camera coords — outside the transform group */}
			{preview && renderShape(preview)}
		</svg>
	);
};

export default DrawingCanvasTransform;
